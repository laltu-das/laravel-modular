...
final class LaravelModularServiceProvider extends ServiceProvider
{
    ...
    public function boot(ModuleRepository $repository): void
    {
        ...
        if ((bool) config('laravel-modular.enabled', true)) foreach ($repository->all() as $module) {
            // Module database directories intentionally follow Laravel's lowercase
            // convention, so register their class namespaces explicitly.
            foreach (ClassLoader::getRegisteredLoaders() as $loader) {
            ...
            if ($module->has('resources/views')) $this->loadViewsFrom($module->path('resources/views'), strtolower($module->name));
            if ($module->has('resources/lang')) $this->loadTranslationsFrom($module->path('resources/lang'), strtolower($module->name));
